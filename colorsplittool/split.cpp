...
#pragma omp parallel for private(i)
    ...
    {
        myCV::normalize(img, img, 0, 255, myCV::normType::MINMAX);
        int i, j, k;
#pragma omp parallel for private(i, k)
        for(j = 0; j < img.rows; j++)
        {
            for(i = 0; i < img.cols; i++)
            {
                for(k = 0; k < 3; k++)
                {
                    img_set[1].at<cv::Vec3b>(j, i)[k] = img.at<cv::Vec3f>(j, i)[0];
                    img_set[2].at<cv::Vec3b>(j, i)[k] = img.at<cv::Vec3f>(j, i)[1];
                    img_set[3].at<cv::Vec3b>(j, i)[k] = img.at<cv::Vec3f>(j, i)[2];
                }
            }
        }
        return;
...
#pragma omp parallel for private(i)
        for(j = 0; j < img.rows; j++)
        {
            for(i = 0; i < img.cols; i++)
            {
                 img_set[1].at<cv::Vec3b>(j, i)[2] = img.at<cv::Vec3f>(j, i)[0];
                 img_set[2].at<cv::Vec3b>(j, i)[1] = img.at<cv::Vec3f>(j, i)[1];
                 img_set[3].at<cv::Vec3b>(j, i)[0] = img.at<cv::Vec3f>(j, i)[2];
            }
        }
        return;
    }
