...
#include <QFileDialog>
#include <QMenu>
PTZLabel::PTZLabel(QWidget *parent) :
    QLabel(parent)
{
...
void PTZLabel::mousePressEvent(QMouseEvent *ev)
{
...
void PTZLabel::mouseReleaseEvent(QMouseEvent *ev)
{
...
void PTZLabel::on_saveAction_triggered()
{
    ...
    {
        int num_index = 0;
        while(QFile::exists(file_name + QString::number(num_index)+".jpg"))
            ++num_index;
        QString file_name = file_name + QString::number(num_index)+".jpg" ;
        _img.save(file_name);


    }
    else{
        QMessageBox::information(0, 0, "Can Not Save Image!!");
    }
}
